...
from opus import Opus, app, config, db, lang, userbot
...
async def eval_handler(_, message: types.Message):
    ...
    response = message.lang["eval_out"].format(
        escape(output)
    )
