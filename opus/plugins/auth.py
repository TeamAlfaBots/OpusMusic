...
async def _auth(_, m: types.Message):
    ...
    if m.command[0] == "auth":
        ...
        await m.reply_text(
            m.lang["auth_added"].format(user.mention)
        )
        ...
        await m.reply_text(
            m.lang["auth_removed"].format(user.mention)
        )
...
async def _authlist(_, m: types.Message):
    ...
    for i, user in enumerate(auth, start=1):
        auth_txt += (
            f"\n{i}. "
            f"<a href=tg://user?id={user}>{user}</a>"
        )
...
@app.on_message(
    filters.command(["admincache", "reload"])
    & filters.group
    & ~app.bl_users
)
@lang.language()
async def _admincache(_, m: types.Message):
    ...
    if m.from_user.id in rel_hist:
        ...
        if time.time() < rel_hist[m.from_user.id]:
            return await m.reply_text(
                m.lang["admin_cache_wait"]
            )

    rel_hist[m.from_user.id] = time.time() + 600

    sent = await m.reply_text(
        m.lang["admin_cache_reloading"]
    )
